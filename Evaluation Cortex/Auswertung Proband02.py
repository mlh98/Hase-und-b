...
def save_results(path_nifti, filename_nifti_b1_list, filename_nifti_b2_list, 
    ...
    for i, file_b1 in enumerate(filename_nifti_b1_list): 
        ...
        segmentation_medulla = np.zeros_like(segmentation)
        cortex_thickness = 3
        
        for slice_ind in slices:
            for b_ind in range(segmentation.shape[3]):
                segmentation_medulla[:,:,slice_ind,b_ind] = binary_erosion(segmentation[:,:,slice_ind,b_ind], iterations=cortex_thickness)    

                # Check if segmentation still contains values
                # Otherwise decrease cortex thickness
                loop=1
                while np.max(segmentation_medulla[:,:,slice_ind,b_ind])<1 and loop<cortex_thickness:
                    segmentation_medulla[:,:,slice_ind,b_ind] = binary_erosion(segmentation[:,:,slice_ind,b_ind], iterations=cortex_thickness-loop)
                    loop=loop+1
                    
                    if np.max(segmentation_medulla[:,:,slice_ind,b_ind])<1 and cortex_thickness-loop<10e-09:
                        # Make erosion weaker by reducing structure size form 3x3 to 2x2
                        segmentation_medulla[:,:,slice_ind,b_ind] = binary_erosion(segmentation[:,:,slice_ind,b_ind], structure=generate_binary_structure(2, 2), iterations=1)
                    else:
                        pass
                    
        # Substract segmentations to obtain cortex segmentation
        segmentation_cortex = segmentation - segmentation_medulla
            
        masked_values = mask(data, segmentation_cortex)
...
path_nifti = os.path.join(meas_dir, '02', 'data')
path_segmentation = os.path.join(meas_dir, '02', 'segmentation')
...
path_save_45_l_bi = os.path.join(results_dir, 'Cortex', 'links', '45_bi')
path_save_60_l_bi = os.path.join(results_dir, 'Cortex', 'links', '60_bi')
path_save_75_l_bi = os.path.join(results_dir, 'Cortex', 'links', '75_bi')
path_save_90_l_bi = os.path.join(results_dir, 'Cortex', 'links', '90_bi')

path_save_45_r_bi = os.path.join(results_dir, 'Cortex', 'rechts', '45_bi')
path_save_60_r_bi = os.path.join(results_dir, 'Cortex', 'rechts', '60_bi')
path_save_75_r_bi = os.path.join(results_dir, 'Cortex', 'rechts', '75_bi')
path_save_90_r_bi = os.path.join(results_dir, 'Cortex', 'rechts', '90_bi')

# Paths for saving results (triexponential)
path_save_45_l_tri = os.path.join(results_dir, 'Cortex', 'links', '45_tri')
path_save_60_l_tri = os.path.join(results_dir, 'Cortex', 'links', '60_tri')
path_save_75_l_tri = os.path.join(results_dir, 'Cortex', 'links', '75_tri')
path_save_90_l_tri = os.path.join(results_dir, 'Cortex', 'links', '90_tri')

path_save_45_r_tri = os.path.join(results_dir, 'Cortex', 'rechts', '45_tri')
path_save_60_r_tri = os.path.join(results_dir, 'Cortex', 'rechts', '60_tri')
path_save_75_r_tri = os.path.join(results_dir, 'Cortex', 'rechts', '75_tri')
path_save_90_r_tri = os.path.join(results_dir, 'Cortex', 'rechts', '90_tri')
